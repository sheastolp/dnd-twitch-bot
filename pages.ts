...
import { modifier, formatRaceName, escapeHtml, formatCentralDateTime } from "./utils.ts";
...
export function renderLogsPage(rows: any[]) {
  const lines = rows
    .map(
      (r: any) =>
        `<tr><td>${escapeHtml(formatCentralDateTime(Number(r.created_at)))}</td><td>${escapeHtml(r.username || "unknown")}</td><td>${escapeHtml(r.broadcaster_id || "")}</td><td><code>${escapeHtml(r.action || "")}</code></td><td>${escapeHtml(r.detail || "")}</td></tr>`,
    )
    .join("");
...
export function renderAdminLogsPage(opts: {
  ...
  const fmt = (ts: any) => (ts ? escapeHtml(formatCentralDateTime(Number(ts))) : "—");
  ...
  const npcOverviewRows = npcOverview
    .map((r: any) => {
      ...
      const chatterOn = Number(r.chatter_enabled) === 1;
      const connected = Number(r.connected) === 1;
      const blocked = Number(r.blocked) === 1;
      const botOn = Number(r.bot_enabled) === 1;
      let why = "";
      ...
      return `<tr class="${rowClass}"><td>${escapeHtml(r.display_name || r.login || r.broadcaster_id)}</td><td>${npcOn ? "on" : "off"}</td><td>${chatterOn ? "on" : "off"}</td><td>${Number(r.character_count ?? 0)}</td><td>${Number(r.total_uses ?? 0)}</td><td>${connected ? "yes" : "no"}</td><td>${blocked ? "yes" : "no"}</td><td>${botOn ? "yes" : "no"}</td><td>${escapeHtml(why)}</td></tr>`;
    })
    .join("");
  ...
  const npcOverviewCard = `<div class="card">
    ...
    ${
      npcOverview.length
        ? `<div style="overflow:auto"><table><thead><tr><th>Channel</th><th>NPCs</th><th>Chatter</th><th>Roster size</th><th>Total uses</th><th>Connected</th><th>Blocked</th><th>Bot on</th><th>Diagnosis</th></tr></thead><tbody>${npcOverviewRows}</tbody></table></div>`
  ...
  return `<h1>🪵 GuildScribe Operator Logs</h1><p class="muted">Auto-refreshes every 30s. Bookmark this URL with your key to check back later. All times shown in Central Time (CST/CDT).</p>${statusCard}${overviewCard}${npcOverviewCard}${eventsCard}<style>
